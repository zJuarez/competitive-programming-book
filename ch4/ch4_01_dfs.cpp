...
void floodfill(int u, int color) {
  ...
  for (int j = 0; j < (int)AdjList[u].size(); j++) {
    ii v = AdjList[u][j];
    if (dfs_num[v.first] == DFS_WHITE) floodfill(v.first, color);
  }
}
...
void dfs2(int u) {  // change function name to differentiate with original dfs
  ...
  for (int j = 0; j < (int)AdjList[u].size(); j++) {
    ii v = AdjList[u][j];
    if (dfs_num[v.first] == DFS_WHITE) dfs2(v.first);
  }
...
int main() {
  ...
  */
  ...
  for (int i = 0; i < V; i++) printf("Vertex %d has color %d\n", i, dfs_num[i]);
  ...
  printThis(
      "Articulation Points & Bridges (the input graph must be UNDIRECTED)");
  dfsNumberCounter = 0;
  dfs_num.assign(V, DFS_WHITE);
  dfs_low.assign(V, 0);
  dfs_parent.assign(V, -1);
  ...
  for (int i = 0; i < V; i++)
    if (articulation_vertex[i]) printf(" Vertex %d\n", i);
